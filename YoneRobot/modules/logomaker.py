...
async def lego(event):
 ...
 try:
    ...
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By Rikka-Chan")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Report @Alvin_Image_Editor_Group, {e}')
...
async def lego(event):
 ...
 if event.sender_id == OWNER_ID:
    ...
    if not quew:
       ...
       pass
 await event.reply('Riika creating your logo...wait!')
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./YoneRobot/resources/blackbg.jpg')
    draw = ImageDraw.Draw(img)
...
__help__ = """
 ❍ /logo text :  Make a logo from your request
